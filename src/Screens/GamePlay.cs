...
namespace CivOne.Screens
{
	...
	internal class GamePlay : BaseScreen
	{
		...
		private void MenuBarWorld(object sender, EventArgs args)
		{
			...
			_gameMenu.Items.Add("Top 10 Cities (F8)").OnSelect((s, a) => Common.AddScreen(new TopCities()));;
		...
		private void MenuBarCivilopedia(object sender, EventArgs args)
		{
			...
			_gameMenu.Items.Add("Wonders of the World").OnSelect((s, a) => Common.AddScreen(new Civilopedia(Civilopedia.Wonders)));
		...
		public GamePlay()
		{
			...
			SetOriginalColours(); // snapshot after Cassette merges so DefaultPalette includes all tokens
