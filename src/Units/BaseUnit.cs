...
namespace CivOne.Units
{
	internal abstract class BaseUnit : BaseInstance, IUnit
	{
		...
		protected virtual bool Confront(int relX, int relY)
		{
			...
			//
			// ...and every unarmed aircraft and ship. The Dirigible, Transport, Longboat and
			// Hydro Engineer, moved by hand onto a foreign unit, fought it at strength 0 and
			// lost their cargo with them. (Sea Caravan, Caravan and Diplomat override this.)
			if (Attack == 0)
			{
				return false;
			}
		...
		// hint that a rule had fired.
		//
		// MoveTo already decides that correctly and dispatches to Confront BEFORE it
